...
let build_multiobjective_fitness json =
    ...
    (module struct
        type t = float*float*float*float*int;; (** The fitness value contains in order, the proportion of true positive, the proportion of true negative, the partial match value of the positive examples, the partial match value of the negative examples and the size of the regexp. *)

        type individual = RegexpTree.t
        type target_data = ExampleList.t

        let to_float (fullpos,fullneg,parpos,parneg,len) =
            (fullpos *. full_example_score) +. (fullneg *. full_counter_example_score) +.
        ...
        let to_string ((fullpos,fullneg,parpos,parneg,len) as f) =
            Printf.sprintf "%.4f (full:%.2f%%+ %.2f%%- | char:%.2f%%+ %.2f%%- | len:%d)" (to_float f) (100. *. fullpos) (100. *. fullneg) (100. *. parpos) (100. *. parneg) len
        ;;

        let compare x y = Pervasives.compare (to_float x) (to_float y);;

        let compute (examples,counter_examples) regexp =
            if RegexpTree.size regexp > elim_size then (0., 0., 0., 0., RegexpTree.size regexp)
            else
            (
                ...
                (float_of_int !fullpos /. float_of_int (Array.length examples),
                float_of_int !fullneg /. float_of_int (Array.length counter_examples),
                !parpos /. float_of_int (Array.length examples),
                !parneg /. float_of_int (Array.length counter_examples),
                RegexpTree.size regexp)
            )
        ;;
